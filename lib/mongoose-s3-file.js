...
var uploadB64ToS3 = function(fileOptions, cb) {
    ...
    var buf = new Buffer(fileOptions.file.replace(/^data:[a-z]*\/\w+;base64,/, ""), 'base64');
        ...
        async.mapSeries(_.keys(document), function(key, cb) {
            if (schema[key]) {
                if (matchS3File(schema[key])) {
                    ...
                    var filename = document['data'] && document['data']['name'] || '';
                    var schemaOptions = extractOptions(schema[key]),
                        fileOptions = {
                            name: (schemaOptions.name ? schemaOptions.name.call(document) : (filename ? guid()+'/'+filename : guid())),
                            file: document[key]
                        };
